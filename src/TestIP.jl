module TestIP
using Toolips

# welcome to your new toolips project!
"""
home(c::Connection) -> _
--------------------
The home function is served as a route inside of your server by default. To
    change this, view the start method below.
"""
function home(c::Connection)
    divip = div("ipdiv")
    usragent = div("uadiv")
    ipa = a("ipa", text = getip(c))
    uaa = a("uaa", text = string(c.http.message["User-Agent"]))
    push!(divip, h("iplabel", 2, text = "found IP:"), ipa)
    push!(usragent, h("ia", 2, text = "found Agent:"), uaa)
    write!(c, DOCTYPE())
    write!(c, divip, usragent)
end

fourofour = route("404") do c
    write!(c, p("404message", text = "404, not found!"))
end

routes = [route("/", home), fourofour]
extensions = Vector{ServerExtension}([Logger(), ])
"""
start(IP::String, PORT::Integer, ) -> ::ToolipsServer
--------------------
The start function starts the WebServer.
"""
function start(IP::String = "127.0.0.1", PORT::Integer = 8000)
     ws = WebServer(IP, PORT, routes = routes, extensions = extensions)
     ws.start(); ws
end
end # - module
